...
describe('DocumentService', () => {
  ...
  it('should fetch documents with page and size params', (done) => {
    ...
    service.getDocuments(0, 10).subscribe(res => {
      expect(res).toEqual(fakeResponse);
      done();
    });

    const req = httpMock.expectOne(
      r => r.url.includes('/documents') && r.params.get('page') === '0' && r.params.get('size') === '10'
    );
  ...
  it('should return error on HTTP failure', (done) => {
    service.getDocuments(0, 5).subscribe({
      next: () => {
